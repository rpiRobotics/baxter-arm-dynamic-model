function [robot_const, robot_structure] = defineBaxterSingleArmRigid(varargin)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % Inertia Matrices in DHP frames:
    ...
    %MMOI (SDH frames) for motors
    Im1 = 0;
    Im2 = 0;
    Im3 = 0; % Motor inertia (oz in s^2)
    ...
    N1 = 1;
    N2 = 1;
    N3 = 1; % Motor inertia and gear ratios
    
    % Inertia matrices in PoE frames:
    Ic1_1 = Ic1_1_prime;
    ...
    Ic(:,:,7) = Ic7_7;
    for k = 1:7
        I(:,:,k) = Ic(:,:,k) - m(k)*(crossmat(c(:,k))*crossmat(c(:,k)));
    end
    
    
    % Motor inertia corrections (PoE frames)
    I(:,:,1) = I(:,:,1) + Im1*N1^2*(z0*z0.');
    I(:,:,2) = I(:,:,2) + Im1*N1^2*(y0*y0.');
    I(:,:,3) = I(:,:,3) + Im2*N2^2*(x0*x0.');
    I(:,:,4) = I(:,:,4) + Im2*N2^2*(y0*y0.');
    I(:,:,5) = I(:,:,5) + Im3*N3^2*(x0*x0.');
    I(:,:,6) = I(:,:,6) + Im3*N3^2*(y0*y0.');
    I(:,:,7) = I(:,:,7) + Im3*N3^2*(x0*x0.');
